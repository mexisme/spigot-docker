...
namespace :build do
  ...
  task :multiverse => "#{ARTEFACT_DIR}/multiverse-core.tar"
  file "#{ARTEFACT_DIR}/multiverse-core.tar" => ARTEFACT_DIR do
    MyShell::Benchmarked.run(
      "#{PACKER_BUILD} -var 'local-artefact-dir=#{ARTEFACT_DIR}' multiverse.json"
    )
  end

  task :build_tools => "#{ARTEFACT_DIR}/BuildTools.jar"
  file "#{ARTEFACT_DIR}/BuildTools.jar" => ARTEFACT_DIR do
    MyShell::Benchmarked.run(
      "#{PACKER_BUILD} -var 'local-artefact-dir=#{ARTEFACT_DIR}' build-tools.json"
    )
  end

  task :spigot => "#{ARTEFACT_DIR}/spigot-#{VERSION}.jar"
  file "#{ARTEFACT_DIR}/spigot-#{VERSION}.jar" => ARTEFACT_DIR do
    MyShell::Benchmarked.run(
